...
-- SQLINES LICENSE FOR EVALUATION USE ONLY
CREATE TABLE jsh_region_country (
    region_id   BIGINT NOT NULL,
    country_id  BIGINT NOT NULL,
    tbl_last_dt DATETIME(6)
);

ALTER TABLE jsh_region_country ADD CONSTRAINT jsh_region_country_pk PRIMARY KEY ( region_id,
                                                                                  country_id );
...
-- SQLINES DEMO *** ength exceeds maximum allowed length(30)
ALTER TABLE jsh_region_country
    ADD CONSTRAINT jsh_region_country_jsh_country_fk FOREIGN KEY ( country_id )
        REFERENCES jsh_country ( country_id );

-- SQLINES DEMO *** ength exceeds maximum allowed length(30)
ALTER TABLE jsh_region_country
    ADD CONSTRAINT jsh_region_country_jsh_region_fk FOREIGN KEY ( region_id )
...
-- # insert and update trigger for region_country intersection table

DELIMITER &&
create trigger TI_REGION_COUNTRY_default_date
    before insert
    on jsh_region_country
    for each row
begin
    IF new.tbl_last_dt IS NULL THEN
        SET new.tbl_last_dt = current_timestamp();
...
DELIMITER &&
create trigger TU_REGION_COUNTRY_default_date
    before update
    on jsh_region_country
    for each row
begin
